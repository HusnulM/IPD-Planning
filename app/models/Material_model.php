...
class Material_model{
  ...
  private $db;
    ...
    public function getBarangByModel($model)
    {
      $this->db->query("SELECT * FROM t_material WHERE matdesc='$model'");
		  return $this->db->single();
    }
    ...
    public function  save($data){
        ...
        $query = "INSERT INTO t_material (material,matdesc,matunit,hourly_target_qty, board_qty,createdon,createdby) 
                      VALUES(:material,:matdesc,:matunit,:hourly_target_qty,:board_qty,:createdon,:createdby)";
        $this->db->query($query);
        ...
        $this->db->bind('board_qty',  $data['boardqty']);
    ...
    public function  update($data){
      $currentDate = date('Y-m-d h:m:s');
        $query = "INSERT INTO t_material (material,matdesc,matunit,hourly_target_qty,board_qty,createdon,createdby) 
        VALUES(:material,:matdesc,:matunit,:hourly_target_qty,:board_qty,:createdon,:createdby)
              ON DUPLICATE KEY UPDATE matdesc=:matdesc,matunit=:matunit,hourly_target_qty=:hourly_target_qty, board_qty=:board_qty";
        $this->db->query($query);
